...
int main(int argc, char** argv){
  ...
  paramLoad.param<std::string>("cmd_vel" ,cmd_vel, "/tb3_2/cmd_vel");
  paramLoad.param<std::string>("scan", scan,"/tb3_2/scan");
  paramLoad.param<std::string>("base_frame", base_frame, "/tb3_0");
  paramLoad.param<std::string>("base_to_row", base_to_row, "/tb3_2");
  ...
  ros::Subscriber laser = node.subscribe(scan, 1, laserCallBack);
  ...
  while (node.ok()){
    ...
    try{
      listener.lookupTransform(base_to_row, base_frame,
